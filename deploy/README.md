...
v2.0 开发提交为 `1db803e`，尚未部署到 ECS，因此线上实例尚未包含管理后台扩展、手动图谱扫描和静态预渲染构建。本阶段不在服务器配置 `cron`；图谱扫描只由管理员在后台按需触发。

完成上线验收前，请依次执行：部署该提交、创建管理员账号、验收管理后台与 MCP 联网工具；域名购买后再完成 HTTPS 与正式 SEO canonical 配置。
